...
{
    // Source: https://github.com/episerver/netcore-preview/blob/master/Quicksilver/EPiServer.Reference.Commerce.Site/Infrastructure/UsersInstaller.cs
    public class UsersInstaller : IBlockingFirstRequestInitializer
    {
        ...
        public async Task InitializeAsync(HttpContext httpContext) 
        {
            ...
            await CreateUser("optimizely-admin", "optimizely-admin", new[] { Roles.Administrators, Roles.WebAdmins });
        }

        private async Task CreateUser(string username, string email, IEnumerable<string> roles)
        {
            var result = await _uIUserProvider.CreateUserAsync(username, "Pass123!", email, null, null, true);
            if (result.Status == UIUserCreateStatus.Success)
            {
                ...
                var resFromSignIn = await _uISignInManager.SignInAsync(username, "Pass123!");
            }
        }
